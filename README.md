...
`qsm2shade` is an R package to calculate the shadow casting of QSMs (quantitative structure models) created in Matlab using TreeQSM. The package uses `QSM` objects read in using the R package <a href = "https://github.com/zoeschindler/qsm2r">qsm2r</a>. The shade of the tree for one sun position can be plotted using `plot_shade_wood()`. To enable shade estimation of the trees when carrying leaves or flowers, leaves and flowers can be simulated using  `add_items()`. To plot the added items and their shade, `plot_items()` and `plot_shade_items()` can be used. To simulate the shading of a tree with or without added items over time, the function `shade_tree()` can be used. To increase efficiency, the simulation of the shade can be executed using parallel processing. The used time steps should be one hour or less, the radiation data should be given in hourly steps and the energy unit should correspond to the chosen time steps and raster resolution. To summarize the output rasters for different time periods, `shade_summarize()` can be used.
