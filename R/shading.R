...
add_radiation <- function(raster, radiation) {
  ...
  radiation_grid <- lapply(1:length(timesteps), function(idx) {
    ...
    curr_rast <- as.numeric(curr_rad[,"diffuse_energy_per_area"]) + as.numeric(curr_rad[,"direct_energy_per_area"]) * curr_rast
    return(curr_rast)
  })
...
#' result_add_2 <- terra::shift(result, dx = 7, dy = -1)
